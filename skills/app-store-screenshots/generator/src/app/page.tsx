...
function PhoneFrame({ accent, imageDataUrl }: { accent: string; imageDataUrl: string | null }) {
  ...
  return (
    <div style={{ position: 'relative', width: PHONE_W }}>
      ...
      <div style={{
        position: 'relative', borderRadius: FRAME_R, padding: FRAME_BEZEL,
        ...
        <div style={{ borderRadius: SCREEN_R, overflow: 'hidden', background: '#000', position: 'relative' }}>
          <div style={{ position: 'absolute', top: DI_TOP, left: '50%', transform: 'translateX(-50%)', width: DI_W, height: DI_H, background: '#000', borderRadius: DI_H / 2, zIndex: 10, boxShadow: '0 1px 4px rgba(0,0,0,0.6), inset 0 0 1px rgba(255,255,255,0.05)' }}>
            <div style={{ position: 'absolute', right: 32, top: '50%', transform: 'translateY(-50%)', width: CAM_D, height: CAM_D, borderRadius: '50%', background: 'radial-gradient(circle at 35% 35%, #2d2d4a 0%, #161625 45%, #0a0a14 70%, #050508 100%)', boxShadow: '0 0 0 2px #1a1a2e, 0 0 0 3px rgba(60,60,80,0.3), inset 0 0.5px 1px rgba(100,100,150,0.15)' }}>
            ...
            <div style={{ position: 'absolute', left: 36, top: '50%', transform: 'translateY(-50%)', width: 6, height: 6, borderRadius: '50%', background: 'radial-gradient(circle, #1a1a28 0%, #0a0a10 100%)', boxShadow: '0 0 0 1px rgba(40,40,60,0.3)' }} />
          </div>
...
function SlideCopy({ shot, lang }: { shot: ShotDef; lang: Lang }) {
  ...
  return (
    <div style={{ position: 'absolute', top: COPY_AREA_TOP, left: 0, right: 0, height: copyAreaH, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', zIndex: 2, padding: '0 60px' }}>
      ...
      <div style={{ fontSize: 28, fontWeight: 400, color: `${a}70`, marginTop: 20, letterSpacing: isJa ? 4 : 1, fontFeatureSettings: "'palt' 1", textShadow: `0 0 30px ${a}20` }}>{copy.sub}</div>
      <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginTop: 20, flexWrap: 'wrap' as const }}>
        {copy.chips.map((c, i) => (
          <span key={i} style={{ padding: '6px 16px', background: `${a}0C`, border: `1px solid ${a}20`, borderRadius: 100, color: c.desc ? 'rgba(255,255,255,0.45)' : `${a}90`, fontSize: 16, fontWeight: c.desc ? 400 : 700, boxShadow: `0 0 15px ${a}08` }}>
...
export default function Page() {
  ...
  const exp1 = useCallback(async (id: number) => {
    ...
    const opts = {
      ...
      pixelRatio: 1,
      cacheBust: true,
      // App Store Connect rejects PNGs with alpha channel.
      // backgroundColor fills the canvas solid → no transparency.
