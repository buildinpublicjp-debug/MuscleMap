...
13. **オンボーディングv8（最大8ページ）** — SplashView → GoalSelectionPage（複数選択） → FrequencySelectionPage（超回復アニメ） → LocationSelectionPage（ジム/自宅/自重/両方、GIF付き） → ProfileInputPage（経験+体重+ニックネーム統合） → [PRInputPage] → GoalMusclePreviewPage → RoutineBuilderPage（自動ルーティン構築） → RoutineCompletionPage（ハードペイウォール付き完了画面） → 通知許可
...
### オンボーディングv8（最大8ページ + SplashView + 通知許可）
...
- **フロー:**
  ...
  OnboardingV2View（最大8ページ横スワイプ TabView）:
    ...
    Page 5: GoalMusclePreviewPage（分割法プレビュー、Dayタップ→GIFシート）
    Page 6: RoutineBuilderPage（自動ルーティン構築 → RoutineManager に保存）
    Page 7: RoutineCompletionPage（ルーティンサマリー + ハードペイウォール）
      ↓
  NotificationPermissionView（通知許可、筋肉マップ回復アニメーション付き）
      ↓
  アプリ本体へ
  ```
...
- **GoalMusclePreviewPage**: 目標に対応する分割法をプレビュー。Dayタップ→GIF付き種目シート表示
...
│   ├── GoalMusclePreviewPage   ← 実装済み（分割法プレビュー + Dayタップ→GIFシート）
...
│   │   ├── GoalMusclePreviewPage.swift    # 分割法プレビュー（Dayタップ→GIFシート）
