...
- 機能を削除する時は参照を全て追跡する。Views → ViewModels → HomeView参照 → 完了画面参照 → 設定参照 → pbxproj
- 「機能が少ないけど全部高品質」は「機能が多いけど全部微妙」より信頼される
- ルーティン画面は「カタログ」。編集UIを最前面に出さない。Dayカードタップ→シートで編集がiOS的に自然
- シェアカードは種目GIF+重量で「俺のルーティン」が伝わるように
